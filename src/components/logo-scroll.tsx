...
const DoubleScrollingLogos = ({ from = 'home' }: { from?: string }) => {
  return (
    <section className='mt-20 mb-20 flex flex-col items-center justify-center space-y-20 px-4 py-4 sm:px-6 lg:px-8'>
      <SectionHeader
...
const LogoItem = ({ src }: { src: string }) => {
  return (
    <img
      src={src}
      alt='logo'
      className='mx-4 h-auto max-w-sm sm:mx-6 sm:max-w-sm lg:mx-10 lg:max-w-xl'
    />
  );
};
